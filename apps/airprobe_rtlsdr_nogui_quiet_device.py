...
# Title: Airprobe Rtlsdr Nogui Quiet
# Generated: Fri Nov 20 03:43:00 2015
##################################################
...
class airprobe_rtlsdr_nogui_quiet(gr.top_block):

    def __init__(self,device=None, fc=943.6e6, shiftoff=400e3, ppm=0, gain=30, samp_rate=2000000.052982):
        gr.top_block.__init__(self, "Airprobe Rtlsdr Nogui Quiet")
...
if __name__ == '__main__':
    ...
    parser.add_option("-d", "--device", dest="device", type="string", default=None,
        help="Set device [default=%default]")
    (options, args) = parser.parse_args()
    tb = airprobe_rtlsdr_nogui_quiet(fc=options.fc, shiftoff=options.shiftoff, ppm=options.ppm, gain=options.gain, samp_rate=options.samp_rate, device=options.device)
    tb.start()
    tb.wait()
